...
Perubahan dari v2:
  - Mendukung jumlah gambar per request yang FLEKSIBEL (1..N), bukan cuma
    maksimal 2 (front/back). User bisa kirim berapa pun foto jambu yang sama
    untuk digrade sekaligus (misal 3-4 sudut berbeda).
  - Istilah "front"/"back" diganti jadi label generik "image_1", "image_2",
    dst — karena sekarang tidak dibatasi cuma 2 sisi.
  - Ada batas atas opsional (MAX_IMAGES_PER_REQUEST) supaya 1 request tidak
    bisa membanjiri antrian GPU dengan ratusan gambar sekaligus. Set None
    untuk benar-benar tanpa batas.
...
Measured latency per request: ~150-300ms tergantung hardware dan jumlah gambar
"""
...
# Batas jumlah gambar per request (untuk 1 buah jambu yang sama, difoto dari
# beberapa sudut). Set None kalau mau benar-benar tanpa batas.
MAX_IMAGES_PER_REQUEST: Optional[int] = 10
...
class InferenceEngine:
    ...
    @staticmethod
    def _get_laplacian_var(crop: np.ndarray) -> float:
    ...
    def infer(self, img: np.ndarray) -> Optional[dict]:
        ...
        for yolo_conf, box in candidates:
            ...
            crop = self._circle_crop(crop)
             ...
             # ← tambahkan di sini, setelah preprocessing
            lap = self._get_laplacian_var(crop)
            print(f"  📊 Laplacian variance: {lap:.2f}")
...
class InferenceQueue:
    ...
    Item antrian: (images_np, loop, future)
      - images_np : list of (label_str, np.ndarray)   ← sekarang bisa N item
    ...
    # ── proses 1 request (1..N gambar) ───────────────────────────────────────
    def _process(self, images_np: list) -> dict:
        per_image     = []
        valid_results = []

        for label, img in images_np:
            if img is None:
                per_image.append({"label": label, "error": "decode_failed", "valid": False})
                continue

            res = self.engine.infer(img)

            if res is None:
                per_image.append({"label": label, "error": "no_jambu_detected", "valid": False})
                continue
        ...
        # ── agregasi lintas semua foto yang dikirim ──────────────────────────
        score_map   = {"A": 3, "B": 2, "C": 1}
        ...
        return {
            ...
            "n_valid_images":  len(valid_results),
            "ai_result":       per_image,
...
app = FastAPI(title="Jambu Grading API v3", lifespan=lifespan)
...
async def grade(images: list[UploadFile] = File(...)):
    """
    Terima 1 atau lebih foto jambu (buah yang sama, difoto dari beberapa
    sudut/sisi), kembalikan grade A/B/C hasil agregasi seluruh foto.

    Jumlah foto tidak dibatasi ke 2 seperti versi sebelumnya — bebas berapa
    pun (dibatasi hanya oleh MAX_IMAGES_PER_REQUEST demi keamanan server).
    ...
    if MAX_IMAGES_PER_REQUEST is not None and len(images) > MAX_IMAGES_PER_REQUEST:
        raise HTTPException(
            status_code=400,
            detail=f"Maksimal {MAX_IMAGES_PER_REQUEST} gambar per request."
        )
